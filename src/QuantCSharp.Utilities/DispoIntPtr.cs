namespace QuantCSharp.Utilities
{
    ...
    public sealed partial class DispoIntPtr : IDisposable
    {
        ...
        public unsafe static DispoIntPtr AllocSBytes(sbyte[] __Value)
        {
            var dispPtr = Alloc(__Value.Length);
            for (int i=0;i<__Value.Length;i++)
            {
                Marshal.WriteByte(dispPtr.Ptr, i, (byte)__Value[i]);
            }
            return dispPtr;
        }
        ...
        unsafe public static explicit operator string (DispoIntPtr dispoIntPtr)
        {
            return dispoIntPtr == null ? string.Empty : new String((sbyte*)dispoIntPtr._ptr);
        }
